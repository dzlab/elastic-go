...
func chap28() {
	...
	c.Aggs("cars", "transactions").SetMetric(e.Count).AddBucket(e.NewBucket("price").AddMetric(e.Histogram, e.Field, "price").AddMetric(e.Histogram, e.Interval, 20000).AddBucket(e.NewBucket("make").AddTerm(e.Field, "make").AddTerm(e.Size, 1))).Get()

	// chart bar of popular makes, their average price and standard error.
	c.Aggs("cars", "transactions").SetMetric(e.Count).AddBucket(e.NewBucket("makes").AddTerm(e.Field, "make").AddTerm(e.Size, 10).AddBucket(e.NewBucket("stats").AddMetric(e.ExtendedStats, e.Field, "price"))).Get()
}
